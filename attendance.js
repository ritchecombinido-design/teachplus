...
//  LEARNERS
// ====================================================
function openAddLearner(){openModal('modal-addlearner');}
async function saveLearner(){
  const lrn=document.getElementById('al-lrn').value.trim();
  const fullname=document.getElementById('al-name').value.trim();
  const sex=document.getElementById('al-sex').value;
  const gradeLevel=document.getElementById('al-grade').value;
  const section=document.getElementById('al-section').value.trim();
  if(!fullname||!gradeLevel||!section){toast('Please fill required fields','error','fa-exclamation');return;}
  await db.learners.add({lrn,fullname,sex,gradeLevel,section});
...
async function loadAttendance(){
  ...
  attLearners=learners.sort((a,b)=>a.fullname.localeCompare(b.fullname));
...
async function renderAttMatrix(){
  ...
  const rows=attLearners.map((l,idx)=>{
    ...
    return `<tr class="att-row"><td class="att-cell name-col body-name">${l.fullname}</td>${cells}</tr>`;
  }).join('');
